...
import { Trash2, X, Search, Save, Package, Pencil, Upload, Lock, Copy, Terminal } from 'lucide-react'
import {
  ItemSlot, RichTextEditor, lineToComponent, plainText, textureUrl,
  manifestStem, manifestName, manifestLore, manifestTags, manifestExtraKeys, manifestLabel, manifestTextures,
  fetchModels, fetchItems, createItem, updateItem, deleteItem, importItems, giveCommand,
} from './mc'
...
function draftToManifest(d) {
  ...
  if (d.model_stem) components['minecraft:item_model'] = `minecraft:custom/${d.model_stem}`
      ...
      {open && !disabled && (
        <div className="absolute z-40 mt-1 w-[min(34rem,92vw)] bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg shadow-2xl p-3">
          ...
          <div className="mt-2 grid grid-cols-6 gap-2 max-h-96 overflow-auto">
            ...
            {!results.length && (
              <p className="col-span-6 text-xs text-zinc-400 px-1 py-3 text-center">No textures match “{query}”.</p>
            )}
          </div>
        </div>
      )}
...
function ImportModal({ onClose, onImported }) {
  ...
  const run = async () => {
    ...
    try {
      const items = await importItems({ text: text.trim() || undefined, file: file || undefined })
      onImported(items)
      onClose()
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-xl bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-2xl p-5 space-y-3" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between">
          <p className="text-sm font-semibold">Import items</p>
          <button onClick={onClose} className="p-1 rounded-md text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-700"><X size={16} /></button>
        </div>
        <p className="text-xs text-zinc-500 dark:text-zinc-400">
          Paste one or more <code>/give</code> commands (one per line) and/or upload a <code>.schem</code> — every
          custom item found is parsed and saved to the library.
        </p>
        <textarea
          value={text}
          ...
          rows={6}
          spellCheck={false}
          placeholder={'/give @p minecraft:diamond_sword[minecraft:custom_name=\'{"text":"Excalibur"}\'] 1'}
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => fileRef.current?.click()} className="flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800">
            <Upload size={14} /> {file ? file.name : 'Choose .schem…'}
          </button>
          {file && <button onClick={() => setFile(null)} className="text-xs text-zinc-400 hover:text-red-600">clear</button>}
          <input ref={fileRef} type="file" accept=".schem" className="hidden" onChange={(e) => setFile(e.target.files[0] || null)} />
        </div>
        {error && <p className="text-xs text-red-600 dark:text-red-400 font-mono">{error}</p>}
        <div className="flex items-center justify-end gap-2">
          <button onClick={onClose} className="px-4 py-1.5 text-sm rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700">Cancel</button>
          <button
            onClick={run}
...
export default function ItemLibrary() {
  ...
  const save = async () => {
    ...
    try {
      ...
      const saved = draft.id ? await updateItem(draft.id, manifest) : await createItem(manifest)
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-2">
            Create custom items with a texture, name, and lore. Highlight text to make it bold, italic, or colored.
            Drag items onto NPC choices to give them in dialogue.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* ---- Editor ---- */}
        <div ref={editorRef} className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl p-4 space-y-4 scroll-mt-4">
          <div className="flex items-center justify-between">
          ...
          <div className="space-y-1">
            ...
            <RichTextEditor value={draft.name} onChange={setName} singleLine placeholder="" />
          </div>

          {/* Lore — rich text, one line per lore entry */}
          <div className="space-y-1">
            <span className="text-xs font-medium text-zinc-500 dark:text-zinc-400">
              Lore <span className="text-zinc-400">(press Enter for a new line)</span>
            </span>
            <RichTextEditor value={draft.lore} onChange={setLore} placeholder="" />
          </div>
          ...
          {hasAdditional && (
            <div className="flex items-start gap-2 px-3 py-2 rounded-lg border border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 text-xs text-amber-800 dark:text-amber-300">
              ...
              <span>
                Contains additional data ({extraKeys.length} component{extraKeys.length > 1 ? 's' : ''}) — preserved on save:
                <span className="font-mono"> {extraKeys.map((k) => k.replace('minecraft:', '')).join(', ')}</span>
              </span>
            </div>
          )}
        ...
        <div className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl p-4 flex flex-col">
          ...
          {items.length === 0 ? (
            <p className="text-xs text-zinc-400 py-8 text-center">No saved items yet. Create one on the left or import.</p>
          ) : filteredItems.length === 0 ? (
            <p className="text-xs text-zinc-400 py-8 text-center">No items match “{query}”.</p>
          ) : (
                  ...
                  <ItemSlot
                    ...
                    hoverActions={(
                      <div className="absolute -top-1.5 -right-1.5 flex gap-0.5 z-10">
                        <button onClick={() => copyGive(item)} title={copiedId === item.id ? 'Copied!' : 'Copy /give command'}
                          className="p-0.5 rounded bg-zinc-700 text-white hover:bg-zinc-600 shadow">
                          {copiedId === item.id ? <Copy size={11} /> : <Terminal size={11} />}
                        </button>
